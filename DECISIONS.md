...
| D-006 | Proposed — approval required | Retain **Bun** as the canonical package-manager workflow. | The repository has `bun.lock` and `bunfig.toml`, and no npm, pnpm, or Yarn lockfile. npm can run the currently installed dependencies but must not silently become the supported install workflow. | Do not add another lockfile or switch package managers. Resolve B-001 by confirming the supported Bun version and CI/local installation command. |
