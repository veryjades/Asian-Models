...
function toRows(blocks: NewsBodyBlock[]): BlockRow[] {
  return blocks.map((block) => ({ id: newRowId(), block }));
}

function fromRows(rows: BlockRow[]): NewsBodyBlock[] {
...
/** Strip any client-only fields before persisting (defensive). */
export function serializeNewsBodyBlocks(blocks: NewsBodyBlock[]): NewsBodyBlock[] {
  return blocks.map((block) => {
    if (block.type === "heading") {
      return { type: "heading", content: block.content, level: block.level ?? 2 };
    }
    if (block.type === "image") {
      return { type: "image", content: block.content, caption: block.caption ?? "" };
    }
...
export function NewsBlockEditor({
  ...
  const [rows, setRows] = useState<BlockRow[]>(() => toRows(blocks));
  ...
  useEffect(() => {
    ...
    const nextRows = toRows(blocks);
    setRows(nextRows);
    rowsRef.current = nextRows;
  }, [blocks]);

  const commit = (nextRows: BlockRow[]) => {
  ...
  const updateBlock = (index: number, patch: Partial<NewsBodyBlock>) => {
    const next = rows.map((row, i) =>
      i === index ? { ...row, block: { ...row.block, ...patch } as NewsBodyBlock } : row,
    );
    commit(next);
  };
  ...
  const changeBlockType = (index: number, nextType: BlockKind) => {
    ...
    if (!current) return;
    const converted = convertBlock(current.block, nextType);
    const next = rows.map((row, i) => (i === index ? { ...row, block: converted } : row));
    commit(next);
  ...
  const addBlock = () => {
    ...
    const block: NewsBodyBlock =
      addKind === "heading"
        ...
        : { type: "text", content: "" };
    commit([{ id: newRowId(), block }, ...rowsRef.current]);
  };
  ...
  const handlePasteSplit = (
    ...
    const current = rows[index];
    if (!current || (current.block.type !== "text" && current.block.type !== "heading")) return;
    const prefix = current.block.content.trim();
    const firstContent = prefix ? `${prefix}\n${parts[0]}` : parts[0]!;
    const inserted: BlockRow[] = parts.slice(1).map((content) => ({
      id: newRowId(),
      block: { type: "text" as const, content },
    }));
    commit([
      ...rows.slice(0, index),
      { ...current, block: { ...current.block, content: firstContent } as NewsBodyBlock },
      ...inserted,
      ...rows.slice(index + 1),
    ]);
  };
    ...
    <div className="space-y-3">
      <div>
        ...
        <p className="mt-1 text-xs text-white/40">
          在最上方選擇類型後「新增到頂部」，編輯完再以上移／下移或拖曳調到合適位置。貼上多段文字會自動拆段。
        </p>
      </div>

      <div className="sticky top-0 z-20 flex flex-wrap items-center gap-3 border border-white/15 bg-slate-950/95 px-3 py-2 backdrop-blur">
        <select
          value={addKind}
          disabled={disabled}
        ...
        <button
          ...
          disabled={disabled}
          onClick={addBlock}
          className="border border-white/20 bg-white/10 px-3 py-2 text-xs text-white hover:bg-white/15 disabled:opacity-40"
        >
          新增到頂部
        </button>
        <span className="text-[11px] text-white/40">新列會插在第 1 列，再拖到目標位置</span>
        <input
          ...
          onChange={(e) => {
            ...
            void onUploadImage(file).then((url) => {
              if (!url) return;
              commit([
                { id: newRowId(), block: { type: "image", content: url, caption: "" } },
                ...rowsRef.current,
              ]);
            });
          }}
      ...
      <NewsBlockPreview blocks={fromRows(rows)} />
    </div>
  );
}
