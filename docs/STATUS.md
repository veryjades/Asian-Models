...
- Supplied hover assets now complete six identity-consistent pairs: Aoi Takahashi uses the seated hair-movement frame; Lin Wei-Jie uses the same-suit side-profile hat gesture (the beach frame is portfolio-only); Hina Chinen uses the seated studio frame for stronger face/hair continuity (the rainbow frame is portfolio-only); Tanya Lim and Han Min-jae use their reviewed same-person full-body extensions while stronger pose variants are sourced. The remaining seven model cards intentionally have no `hoverPortrait` until genuine same-person pose frames are available.
...
- Identity-consistent hover photography remains in progress for the remaining 9 models. No current asset may be duplicated, mirrored, cropped, or exchanged for another person to imitate the effect.
...
Source and review genuine same-session paired images for the remaining 12 models. Keep PR #5 draft/open/not merged.
