...
- Supplied hover assets now complete four reviewed pairs: Aoi Takahashi uses the seated hair-movement frame; Lin Wei-Jie uses the same-suit side-profile hat gesture (the beach frame is portfolio-only); and Hina Chinen uses the seated studio frame for stronger face/hair continuity (the rainbow frame is portfolio-only). The remaining nine model cards intentionally have no `hoverPortrait` until genuine same-person pose frames are available.
