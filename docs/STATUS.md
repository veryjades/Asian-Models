...
- A fresh owner reset invitation was attempted from the live `/admin` form and again from Supabase Auth Users → “Send password recovery”; both returned `429: email rate limit exceeded` (`over_email_send_rate_limit`). Therefore no new invitation was sent. The application path remains ready for a retry after the provider window clears.
