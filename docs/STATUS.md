...
- Server-only role provisioning is deployed as Supabase Edge Function `provision-user` (version 1, `verify_jwt=true`): missing auth returns 401 and an anonymous/non-admin JWT returns 403 in live smoke checks; the function preserves existing metadata and writes only the validated `app_metadata.role`.
