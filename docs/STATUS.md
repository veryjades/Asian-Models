...
**Last heartbeat:** 2026-08-15 +08:00 (Auth URL correction and local admin QA)
...
- Admin recovery delivery is verified: `src/lib/supabase/auth.ts` supports reset/update password operations and `/admin` renders a first-time setup form with a 12-character minimum. Supabase Auth URL Configuration is set to `http://127.0.0.1:8090` with local and Preview wildcard redirects; the Dashboard confirmed all three URLs were added. The existing `menscheck@gmail.com` account is email-confirmed and now has the server-side `app_metadata.role=admin`; a fresh recovery request completed with HTTP 200 in Auth logs at 2026-08-15 03:39:28 and routes to `/admin?reset=1`. Local browser QA at `/admin` returned the sign-in screen and the `Forgot password / first-time setup` control. The previously issued `localhost:3000` OTP is stale and must not be reused; no password is stored in the repository or browser code.
